...
import { bob, charles, jean, jean2, john, john2, Person } from '../fixtures'
...
describe('Using Entity Adapter', () => {
  ...
  it('should be able to update an entity', () => {
    adapter.addMany(state, [bob, jean])
    adapter.updateOne(state, { id: bob.id, changes: { name: 'Tarzan' } })
  ...
  it('should be able to update several entities', () => {
    adapter.addMany(state, [bob, jean])
    adapter.updateMany(state, [
      { id: bob.id, changes: { name: 'Tarzan' } },
      { id: charles.id, changes: { name: 'Error' } },
    ])
  ...
  it('should be able to upsert an entity', () => {
    adapter.addMany(state, [bob, jean])
    adapter.upsertOne(state, { ...bob, name: 'Tarzan' })
  ...
  it('should be able to upsert several entities', () => {
    adapter.addMany(state, [bob, jean])
    adapter.upsertMany(state, [{ ...bob, name: 'Tarzan' }, charles])

    expect(state).toEqual({
      ids: [charles.id, jean.id, bob.id],
      entities: {
        [bob.id]: {
          id: bob.id,
          name: 'Tarzan',
